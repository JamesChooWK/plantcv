...
function install_dependencies_darwin() {
  if [ -x '/opt/local/bin/port' ]; then
    ...
    sudo port install git sqlite3 opencv +python36 py36-virtualenv
...
function install_dependencies_linux() {
  if [ -x '/usr/bin/apt' ]; then
    ...
    sudo apt-get install git libopencv-dev python-opencv \
      sqlite3 python-setuptools libpython3.6-dev python3-pip virtualenv
  elif [ -x '/usr/bin/yum' ]; then
    echo "Getting dependencies with Yum."
    sudo yum install git libopencv-dev python-opencv sqlite3 \
      python-setuptools libpython3.6-dev python-pip virtualenv
  else
